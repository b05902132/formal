...
Theorem wp_goal :
  ...
  P_array_elem_swapped t t1 a 2%Z 0%Z 1%Z -> P_permutation t t1 a a 2%Z.
(* Why3 intros t t1 a h1 h2 h3. *)
Proof.
Open Scope Z_scope.
Require Import Lia.
intros M1 M2 arr h1 h2 [[_ [swap1 swap2]] noswap].
intros elem elem_i32.
assert (base : L_count_elem M2 elem arr 0 = L_count_elem M1 elem arr 0).
{ repeat rewrite Q_empty; trivial; try lia. }
replace 1 with (1 + 0) in * by lia.
replace 2 with (1 + (1 + 0)) by lia.
destruct (Z.eq_dec elem (M2 (shift arr (1 + 0))));
destruct (Z.eq_dec elem ( M2 (shift arr 0))).
- repeat rewrite <- Q_append_eq; trivial; try lia.
- rewrite <- Q_append_eq; trivial; try lia.
  rewrite Q_append_neq; trivial; try lia.
  rewrite Q_append_neq; trivial; try lia.
  rewrite <- Q_append_eq; trivial; try lia.
- rewrite Q_append_neq; trivial; try lia.
  rewrite <- Q_append_eq; trivial; try lia.
  rewrite <- Q_append_eq; trivial; try lia.
  rewrite Q_append_neq; trivial; try lia.
- repeat rewrite Q_append_neq; trivial; lia.
