...
Theorem wp_goal :
  ...
  (2%Z <= i3)%Z -> ((i + x1)%Z <= 30000%Z)%Z -> ((x2 + x4)%Z <= 30000%Z)%Z ->
...
(* Observing the type of havoc_access and the goal,
   we see that RHS has no t2 and t3,
   so the following replacement has to be valid. *)
...
(* Now we need to prove
   ...
   Considering arr[len] and buf[buf_used] is the same element,
   I doubt if it's possible to prove this.
*)
