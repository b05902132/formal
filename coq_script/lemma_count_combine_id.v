...
Theorem wp_goal :
  ...
  (0%Z <= i1)%Z -> (0%Z <= i)%Z -> is_sint32_chunk t ->
  P_count_combine t a i (shift a i) i1 a.
Proof.
Require Import Lia.
Open Scope Z_scope.
intros t a i i1 h1 h2 h3.
unfold P_count_combine.
intros elem elem_i32.
generalize dependent i1.
apply natlike_ind.
- replace (i + 0) with i by lia.
  rewrite Q_empty with (arr := (shift a i)); trivial; lia.
- intros len len_ge_0 IH.
  replace (Z.succ len) with (1 + len) by lia.
  replace (i + (1 + len)) with (1 + (i + len)) by lia.
  destruct (Z.eq_dec  (t (shift (shift a i) len)) elem);
  [ repeat rewrite <- Q_append_eq; trivial; try lia
   |repeat rewrite Q_append_neq; trivial; try lia ];
  rewrite <- double_shift; trivial.
Qed.
