...
Theorem wp_goal :
  ...
  ((L_power 2%Z i) = (lsl 1%Z i)).
(* Why3 intros i h1. *)
Proof.
Open Scope Z_scope.
(* Lia is the new name of omega strategy.
   In this proof it's used only to prove trival things such like
   if i >= 0 then suc i > 0. *)
Require Import Lia.
apply natlike_ind.
- replace (lsl 1 0) with 1 by (symmetry; apply lsl_1_0).
  replace (L_power 2 0) with 1 by (symmetry; apply L_power_def; lia).
  trivial.
- intros i i_ge_0 IH.
  replace (L_power 2 (Z.succ i)) with (2 * L_power 2 i).
  replace (lsl 1 (Z.succ i)) with (2 * (lsl 1 i)).
  rewrite IH; trivial.
  + (* proving the second replacement about lsl. *)
    replace (Z.succ i) with (i + 1) by lia.
    rewrite <- lsl_add; try lia.
    Search lsl.
    rewrite -> lsl_1.
    trivial.
  + (* proving the first replace. *)
    replace (i) with (- (1) + (Z.succ i)) at 1 by lia; symmetry.
      apply L_power_def. lia.
